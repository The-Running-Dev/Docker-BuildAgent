...
.EXAMPLE
    Copy-Directory -sourceDir './template' -destinationDir './docs-ui' -UpdateGitIgnore
    # Copies files and appends copied file paths to destination .gitignore
.NOTES
    ...
    .gitignore updates are optional and only occur when -UpdateGitIgnore is specified.
#>
    param(
        [Parameter(Mandatory = $true)]
        [ValidateNotNullOrEmpty()]
    ...
    Write-Host "[COPY] Copying: $SourceDir -> $DestinationDir ($mode)" -ForegroundColor Yellow
        ...
        # Prepare optional gitignore management
        ...
        try {
            Get-ChildItem -Path $SourceDir -Recurse | ForEach-Object {
                if (-not $_.PSIsContainer) {
                    ...
                    if ($Overwrite -or -not (Test-Path $destPath)) {
                        ...
                        # Track copied files only when gitignore updates are enabled
                        if ($UpdateGitIgnore) {
            ...
            if ($UpdateGitIgnore -and $copiedFiles.Count -gt 0) {
